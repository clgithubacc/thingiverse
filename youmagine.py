...
class Youmagine(object):
    ...
    def getCategories(self):
        return [n[0] for n in self._categories]

    def getLicenses(self):
        return [n[0] for n in self._licenses]
    ...
    def createDesign(self, name, description, category, license):
        ...
        if len(excerpt) >= 300:
            ...
            excerpt = lines[0]
            description = '\n'.join(lines[1:])
        res = self._request('POST', '/designs.json',
                            {'design[name]': name, 'design[excerpt]': excerpt, 'design[description]': description,
                             'design[design_category_id]': [n for n in self._categories if n[0] == category][0][1],
                             'design[license]': [n for n in self._licenses if n[0] == license][0][1]})
        if 'id' in res:
            return res['id']
        print(res)
        return None
    ...
    def createDocument(self, designId, name, contents):
        res = self._request('POST', '/designs/%d/documents.json' % (designId),
                            {'document[name]': name, 'document[description]': 'Uploaded from Cura'},
    ...
    def createImage(self, designId, name, contents):
        res = self._request('POST', '/designs/%d/images.json' % (designId),
                            {'image[name]': name, 'image[description]': 'Uploaded from Cura'},
                            {'image[file]': (name, contents)})
        if 'id' in res:
            return res['id']
        print(res)
        return None
        ...
        for n in range(0, retryCount):
            ...
            try:
                if files is not None:
                    ...
                    self._http.request(method, url, s, {"Content-type": "multipart/form-data; boundary=%s" % (boundary),
                                                        "Content-Length": len(s)})
                elif postData is not None:
                    self._http.request(method, url, urllib.parse.urlencode(postData),
